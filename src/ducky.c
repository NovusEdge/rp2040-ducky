...
// Sending two reports in the same interval can coalesce and drop a keystroke
// on some hosts, so the runner emits at most one report per interval.
...
void ducky_run(const duck_step_t *steps, size_t count) {
    ...
    case STEP_KEY:
        // Press on one call, release on the next, then advance.
        if (!key_down) {
    ...
    case STEP_STRING:
        ...
        // Release between every character so repeats register.
        if (!key_down) {
